...
from infrastructure.results.inspection_result import get_latest_result_by_cluster
    ...
    def get_nodes(self) -> List[Dict]:
        ...
        for node in self.config["nodes"]:
            ...
            if node_copy.get("password_encrypted"):
                logger.info(
                    f"Password for node {node_copy.get('ip')} is marked as encrypted, but will be used in plain text"
                )
                node_copy["password_encrypted"] = False
...
# Simple cache for cluster status counts
...
def get_cluster_status_counts_fast() -> Dict[str, int]:
    ...
    global _cluster_status_cache, _cache_timestamp
    import time
    ...
    if _cache_timestamp and (current_time - _cache_timestamp) < _CACHE_TTL:
        return _cluster_status_cache.copy()
    ...
    # Update cache
    _cluster_status_cache = counts.copy()
