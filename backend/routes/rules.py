...
def _sync_gitops_repository() -> bool:
    ...
    try:
        ...
        print(f"ERROR: GitOps sync failed: {str(e)}")
        return False
...
def _log_rules_statistics(rules: dict[str, list], use_gitops: bool) -> int:
    ...
    print(f"DEBUG: Loaded {total_rules} rules total (GitOps: {use_gitops})")
    for rule_type, rule_list in rules.items():
        print(f"DEBUG: {rule_type}: {len(rule_list)} rules")
    return total_rules


def _fallback_to_local_rules() -> tuple[dict[str, list], bool]:
    """Execute fallback to local rules"""
    print("DEBUG: GitOps enabled but no rules found, trying local fallback")
...
async def get_rules():
    ...
    try:
        ...
        print(f"DEBUG: GitOps mode: {use_gitops}")
