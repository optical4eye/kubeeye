...
def check_connection(
    node_ip: str, target_ip: str, target_port: int, timeout: int = 5
) -> Dict[str, Any]:
    """
    Check network connectivity from a node to a target IP and port

    Args:
        node_ip: IP address of the node performing the check
...
def check_connectivity_from_nodes(
    ...
    Args:
        nodes: List of node dictionaries with 'ip' and 'name' keys
    ...
    def check_single_node(node):
        ...
        result = check_connection(node_ip, target_ip, target_port, timeout)
        return result
