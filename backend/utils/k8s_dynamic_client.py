...
class K8sDynamicClient(K8sBaseClient):
    ...
    def _analyze_resource_strategy(self, resource_config: List[Dict]) -> Dict[str, Any]:
        ...
        if has_pods and has_controllers:
            ...
            if len(configured_controllers) >= 2:
                ...
                logger.debug(
                    "[_analyze_resource_strategy] Triggered controller-only strategy (multiple controllers+Pod)"
                )
                strategy.update(
                    {
                ...
                logger.debug(
                    "[_analyze_resource_strategy] Applied controller-only strategy, disabled direct Pod acquisition"
                )
            else:
    ...
    def list_resources_from_config_optimized(
        ...
        self.build_resource_mappings_from_config(rule_config)
        logger.debug(
            f" [list_resources_from_config_optimized] Mapping table keys after build: {list(self.resource_mappings.keys())}"
        )
        ...
        logger.debug(
            f" [list_resources_from_config_optimized] Resources extracted from config.resources: {[r.get('kind', 'unknown') for r in resource_config]}"
        )
        ...
        if not resource_config:
            ...
            logger.debug(
                f" [list_resources_from_config_optimized] Resources extracted from root level resources: {[r.get('kind', 'unknown') for r in resource_config]}"
            )

        strategy = self._analyze_resource_strategy(resource_config)

        logger.info(
            f" [list_resources_from_config_optimized] Selected resource acquisition strategy: {strategy['mode']} - {strategy['reasoning']}"
        )
        logger.debug(
            f" [list_resources_from_config_optimized] Complete strategy information: {json.dumps(strategy, indent=2)}"
        )
        ...
        logger.debug(
            f" [list_resources_from_config_optimized] Step 4: Get resources according to strategy '{strategy['mode']}'"
        )
        ...
        if strategy["mode"] == "controller_only":
            ...
            for i, resource_type in enumerate(controllers_to_get):
                ...
                if resource_type_lower in self.resource_mappings:
                    ...
                    logger.info(
                        f" [list_resources_from_config_optimized] Got {resource_type_lower}: {len(controller_resources)} items"
                    )
                else:
                    logger.warning(
                        f" [list_resources_from_config_optimized] Not found in resource mapping: {resource_type_lower}"
                    )
                    logger.debug(
                        f" [list_resources_from_config_optimized] Available mappings: {list(self.resource_mappings.keys())}"
                    )
            ...
            for i, resource_type in enumerate(controllers_to_get):
                ...
                if resource_type_lower in self.resource_mappings:
                    logger.debug(
                        f" [list_resources_from_config_optimized] Found controller in resource mapping: {resource_type_lower}"
                    )
                    controller_resources = self._get_namespaced_resources(
                        resource_type_lower, include_namespaces, exclude_namespaces
                    )
                    ...
                    logger.info(
                        f" [list_resources_from_config_optimized] Got controller {resource_type_lower}: {len(controller_resources)} items"
                    )
                else:
                    logger.warning(
                        f" [list_resources_from_config_optimized] Controller resource mapping missing: {resource_type_lower}"
                    )
                    logger.debug(
                        f" [list_resources_from_config_optimized] Available mappings: {list(self.resource_mappings.keys())}"
                    )
            ...
            logger.debug(
                f" [list_resources_from_config_optimized] pods whether in mapping: {'pods' in self.resource_mappings}"
            )
            ...
            logger.debug(
                f" [list_resources_from_config_optimized] Built mapping table: {list(self.resource_mappings.keys())}"
            )
            ...
            for i, resource_type in enumerate(self.resource_mappings.keys()):
                ...
                try:
                    ...
                    if is_namespaced:
                        logger.debug(
                            f" [list_resources_from_config_optimized] Processing namespace-level resource: {resource_type}"
                        )
                        # Handle namespace-level resources
                        if include_namespaces:
                            logger.debug(
                                f" [list_resources_from_config_optimized] Getting resources for specified namespaces {include_namespaces}"
                            )
                            # Only get resources for specified namespaces
                            all_resources = []
                            for j, namespace in enumerate(include_namespaces):
                                if namespace not in exclude_namespaces:
                                    ...
                                    logger.debug(
                                        f" [list_resources_from_config_optimized] Skip excluded namespace: {namespace}"
                                    )
                            resources[resource_type] = all_resources
                            logger.debug(
                            ...
                            all_resources = self.list_resources(resource_type)
                            logger.debug(
                                f" [list_resources_from_config_optimized] All namespaces got: {len(all_resources)} items"
                            )
                            if exclude_namespaces:
                                logger.debug(
                                    f" [list_resources_from_config_optimized] Filtering excluded namespaces: {exclude_namespaces}"
                                )
                                filtered_resources = [
                                    res
                                    for res in all_resources
                                ...
                                resources[resource_type] = filtered_resources
                                logger.debug(
                                    f" [list_resources_from_config_optimized] After filtering: {len(filtered_resources)} items (original: {len(all_resources)} items)"
                                )
                            else:
                                resources[resource_type] = all_resources
                                logger.debug(
                                    f" [list_resources_from_config_optimized] No filtering needed: {len(all_resources)} items"
                                )
                    else:
                        logger.debug(
                            f" [list_resources_from_config_optimized] Processing cluster-level resource: {resource_type}"
                        )
                        # Cluster-level resources, get directly
                        cluster_resources = self.list_resources(resource_type)
                        resources[resource_type] = cluster_resources
                        logger.debug(
                            f" [list_resources_from_config_optimized] Cluster-level got: {len(cluster_resources)} items"
                        )

                    logger.info(
                        f" [list_resources_from_config_optimized] Got {resource_type}: {len(resources[resource_type])} resources"
                    )
        ...
        try:
            ...
            if include_namespaces:
                ...
                for i, namespace in enumerate(include_namespaces):
                    ...
                    if namespace not in exclude_namespaces:
                        logger.debug(
                            f" [_get_namespaced_resources] Namespace {namespace} not excluded, starting acquisition"
                        )
                        logger.debug(
                ...
                if exclude_namespaces:
                    ...
                    logger.debug(
                        f" [_get_namespaced_resources] Filtering completed: {len(filtered_resources)} items (original: {len(all_resources)} items)"
                    )
                    return filtered_resources
                else:
                    logger.debug(
                        f" [_get_namespaced_resources] No filtering needed, return all resources: {len(all_resources)} items"
                    )
                    return all_resources
