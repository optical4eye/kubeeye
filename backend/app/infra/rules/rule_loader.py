...
class Rule:
    ...
    def __init__(self, rule_data: Dict):
        ...
        self.type = rule_data.get("type", "")  # Rule type: node, opa
...
# Global cache for rules with metadata using CacheManager
_cache_manager = CacheManager()
_rules_cache = _cache_manager.get_or_create_cache("rules", maxsize=128, ttl=300)  # 5 minutes TTL
...
def clear_rules_cache():
    """Clear rules loading cache"""
    # Clear in-memory cache using CacheManager
    _cache_manager.invalidate_namespace("rules")
    _cache_manager.invalidate_namespace("rules_metadata")
