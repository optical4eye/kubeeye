...
def get_cluster_cert_status(cluster_name: str, kubeconfig_content) -> Dict[str, any]:
    """
    Check cluster certificate status

    Args:
        cluster_name: cluster name
        kubeconfig_content: kubeconfig file content (string) or dict
    ...
    try:
        ...
        # Parse kubeconfig - handle both string and dict input
        if isinstance(kubeconfig_content, str):
            kubeconfig = yaml.safe_load(kubeconfig_content)
        elif isinstance(kubeconfig_content, dict):
            kubeconfig = kubeconfig_content
        else:
            return {
                "status": "unknown",
                "days_remaining": None,
                "error": f"invalid kubeconfig content type: {type(kubeconfig_content)}",
            }
        if not kubeconfig:
            return {
                "status": "unknown",
            ...
            if cert_data:
                ...
                if cert_info:
                    ...
                    details["client_certificates"].append(
                        {"user": user_name, "type": "client-certificate-data", **status}
                    )
            elif cert_path:
