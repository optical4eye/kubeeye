    ...
    def _check_single_node_connection_with_timeout(self, node: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                ...
                success, message = future.result(timeout=self.connection_check_timeout)
                return success, message

        except concurrent.futures.TimeoutError:
            return (
                False,
                f"Connection timeout ({self.connection_check_timeout}s) for node {node_name}",
            )
        except Exception as e:
