...
class GitOpsRuleManager:
    ...
    def save_config(self, config: Dict):
        ...
        if config.get("from_env") or (config.get("repository") and config["repository"].get("from_env")):
            print("Configuration is managed via environment variables and cannot be changed through the interface")
            return
    ...
    def get_repo_rules(self, repo_name: str):
        ...
        print(f"Loaded {len(rules)} rules from Git repository {repo_name}, all automatically enabled")
        return rules
    ...
    def set_repository(self, repo_config: Dict) -> Tuple[bool, str]:
        ...
        if config.get("from_env") or (config.get("repository") and config["repository"].get("from_env")):
            return False, "Repository is managed via environment variables and cannot be changed"
    ...
    def remove_repository(self) -> Tuple[bool, str]:
        ...
        # Check if repository is managed via ENV
        if config.get("from_env") or (config.get("repository") and config["repository"].get("from_env")):
            return False, "Repository is managed via environment variables and cannot be deleted"
