...
4. Check network availability of port {node_port}"""
        ...
        result = self._result_formatter.error_result(
            rule,
            error_msg,
            f"Failed to establish SSH connection with node {node_name}"
        )
...
class NodeInspector(BaseInspector):
    ...
    def run_inspection(self, cluster_name: str, rule_ids: List[str] = None) -> InspectionResult:
        ...
        logger.info(f"Statistics: {self.execution_stats['available_nodes']} available, {self.execution_stats['unavailable_nodes']} unavailable nodes")
    ...
    def _check_all_node_connections(self) -> List[Dict]:
        ...
        logger.info(f"Connection check completed. Available: {len(available_nodes)}, Unavailable: {len(self.nodes) - len(available_nodes)}")
    ...
    def _format_execution_error_result(self, rule: Rule, node: Dict, error_msg: str) -> Optional[Dict]:
        ...
        result = self.rule_processor.result_formatter.error_result(
            rule,
            error_msg,
            f"Execution error on node {node_name}"
        )
        result['node'] = {'ip': node['ip'], 'name': node_name}
        result['name'] = f"{rule.name} - {node_name}"
        return result
